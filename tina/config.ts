// TinaCMS configuration for the Homebuyers SC website.
//
// This file tells the web editor at /admin what it is allowed to edit and
// which fields to show. It does NOT change how the site is built or served:
// the site stays a fully static Astro build. Tina reads and writes the same
// markdown files in src/content that the site has always used, and saves by
// committing to GitHub, which makes Netlify rebuild.
//
// The clientId below is public by design and safe to keep in the repo.
// The token is a secret and is read from an environment variable instead.

import { defineConfig } from 'tinacms';

// On Netlify the HEAD variable holds the branch being built.
const branch = process.env.HEAD || 'main';

// The nine buckets guides are grouped into. These are a fixed list on purpose:
// a typo in this value silently drops a guide out of its group on the site.
const GUIDE_GROUPS = [
...
// Shared field definitions, so the three collections stay consistent.
const titleField = {
...
const oldUrlField = {
  ...
  description:
    'A record of where this page lived on the old site. Changing it does nothing good and loses the history. Leave it alone.',
};
    ...
    collections: [
      ...
      },
      {
        ...
        ui: {
          ...
          filename: {
            readonly: true,
            slugify: (values: { city?: string }) =>
              'cash-home-buyers-' +
              (values?.city || 'new-city').toLowerCase().replace(/[^a-z0-9]+/g, '-') +
              '-sc',
          },
        },
        fields: [
